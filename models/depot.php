...
class Depot {
    ...
    public function getMainDepotId(): int {
    $stmt = $this->db->query("SELECT id FROM depots WHERE is_principal = 1 LIMIT 1");
    $id = $stmt->fetchColumn();
    if (!$id) throw new RuntimeException("Aucun dépôt principal (is_principal=1).");
    return (int)$id;
}
}